#!/usr/bin/env python3
"""Startup script for the Demo API example.

Starts the Task Manager API backend, OMCP server, and optionally the Chainlit UI.
...
def main():
    parser = argparse.ArgumentParser(description="Start the Demo API example")
    parser.add_argument(
        "--ui",
        action="store_true",
    ...
    print("=" * 50)
    print("Task Manager Demo - Startup Script")
    print("=" * 50)
    print()

    # Start Task Manager API backend
    ...
    if args.ui:
        ui_proc = start_process(
            "Chainlit UI",
            ["uv", "run", "chainlit", "run", "examples/demo_api/app.py", "--port", "8001"],
            8001,
        )
    ...
    print("=" * 50)
    print()
    print("  Task API:   http://localhost:8000")
    print("  OMCP:       http://localhost:9000")
    if args.ui:
        print("  Chainlit:   http://localhost:8001  <- Open this in your browser!")
    print()
    if not args.ui:
        print("To start the web UI, restart with --ui flag:")
        print("  uv run python examples/demo_api/start.py --ui")
    print()
